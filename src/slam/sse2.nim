...
# template MM_SHUFFLE*(z, y, x, w: int | uint | float32): int32 =
#   ((int(z) shl 6) or (int(y) shl 4) or (int(x) shl 2) or int(w)).int32
...
template m128*(): M128 = mm_setzero_ps()

template m128*(a: float32): M128 = mm_set1_ps(a)

template m128d*(): M128d = mm_setzero_pd()

template m128i*(): M128i = mm_setzero_si128()

template m128d*(a: float64): M128d = mm_set1_pd(a)

template m128i*(a: int32): M128i = mm_set1_epi32(a)

template `and`*(a, b: M128): M128 = mm_and_ps(a, b)

template `and`*(a, b: M128d): M128d = mm_and_pd(a, b)

template `and`*(a, b: M128i): M128i = mm_and_si128(a, b)

template `or`*(a, b: M128): M128 = mm_or_ps(a, b)

template `or`*(a, b: M128d): M128d = mm_or_pd(a, b)

template `or`*(a, b: M128i): M128i = mm_or_si128(a, b)

template `xor`*(a, b: M128): M128 = mm_xor_ps(a, b)

template `xor`*(a, b: M128d): M128d = mm_xor_pd(a, b)

template `xor`*(a, b: M128i): M128i = mm_xor_si128(a, b)

template `==`*(a, b: M128): M128 = mm_cmpeq_ps(a, b)

template `==`*(a, b: M128d): M128d = mm_cmpeq_pd(a, b)

template `!=`*(a, b: M128): M128 = mm_cmpneq_ps(a, b)

template `!=`*(a, b: M128d): M128d = mm_cmpneq_pd(a, b)

template `>`*(a, b: M128): M128 = mm_cmpgt_ps(a, b)

template `>`*(a, b: M128d): M128d = mm_cmpgt_pd(a, b)

template `>=`*(a, b: M128): M128 = mm_cmpge_ps(a, b)

template `>=`*(a, b: M128d): M128d = mm_cmpge_pd(a, b)

template `<`*(a, b: M128): M128 = mm_cmplt_ps(a, b)

template `<`*(a, b: M128d): M128d = mm_cmplt_pd(a, b)

template `<=`*(a, b: M128): M128 = mm_cmple_ps(a, b)

template `<=`*(a, b: M128d): M128d = mm_cmple_pd(a, b)

template `+`*(a, b: M128): M128 = mm_add_ps(a, b)

template `+`*(a, b: M128d): M128d = mm_add_pd(a, b)

template `+=`*(a: var M128, b: M128) = a = a + b

template `+=`*(a: var M128d, b: M128d) = a = a + b

template `-`*(a, b: M128): M128 = mm_sub_ps(a, b)

template `-`*(a, b: M128d): M128d = mm_sub_pd(a, b)
...
template `*`*(a, b: M128d): M128d = mm_mul_pd(a, b)

template `*=`*(a: var M128, b: M128) = a = a * b

template `*=`*(a: var M128d, b: M128d) = a = a * b

template `/`*(a, b: M128): M128 = mm_div_ps(a, b)

template `/`*(a, b: M128d): M128d = mm_div_pd(a, b)

template `/=`*(a: var M128, b: M128) = a = a / b

template `/=`*(a: var M128d, b: M128d) = a = a / b

template max*(a, b: M128): M128 = mm_max_ps(a, b)

template max*(a, b: M128d): M128d = mm_max_pd(a, b)

template min*(a, b: M128): M128 = mm_min_ps(a, b)

template min*(a, b: M128d): M128d = mm_min_pd(a, b)

template sqrt*(a: M128): M128 = mm_sqrt_ps(a)

template sqrt*(a: M128d): M128d = mm_sqrt_pd(a)
